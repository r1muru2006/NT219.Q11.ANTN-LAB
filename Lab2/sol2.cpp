...
int main() {
    ...
    string plain = "This is my homework in Lab 2";
    string cipher;
    ...
    vector<unsigned char> prev_block(iv, iv + AES::BLOCKSIZE);
    string recovered;

    for (size_t i = 0; i < cipherVec.size(); i += AES::BLOCKSIZE) {
        unsigned char current_block[AES::BLOCKSIZE];
        memcpy(current_block, cipherVec.data() + i, AES::BLOCKSIZE);
        cout << "\nBLOCK " << (i / AES::BLOCKSIZE) + 1 << endl;
        
        string current_block_hex;
        StringSource(current_block, AES::BLOCKSIZE, true, new HexEncoder(new StringSink(current_block_hex)));
        cout << "Cipher block " << (i / AES::BLOCKSIZE) + 1 << ": " << current_block_hex << endl;

        vector<unsigned char> original_cipher_block(current_block, current_block + AES::BLOCKSIZE);
        ...
        for (size_t j = 0; j < AES::BLOCKSIZE; j++)
        ...
        recovered.append((char*)current_block, AES::BLOCKSIZE);
